...
/* ==================================================================
   AI MCQ ENGINE — LICENSE GATE  (Phase 5)
   ==================================================================
   This block is injected at the TOP of aimcq.js, immediately after
   `window.AIMCQ_CONFIG = window.AIMCQ_CONFIG || {};`.

   It is reproduced here as a standalone reference so you can read,
   diff, or re-inject it. The live copy lives inside aimcq.js — edit
   there, not here.

   WHAT IT DOES
     • Verifies an RS256-signed license token entirely in the browser
       using window.crypto.subtle — ZERO network calls, ever.
     • Binds the token to the page's hostname and an expiry date.
     • Gates initAimcqQuiz and loadAimcqFromDrive: a quiz only renders
       after the token verifies. On failure every quiz container shows
       a styled "license required" panel instead.
     • Fails CLOSED — missing token, old browser, or any error blocks
       the quiz. The engine never silently runs unlicensed.

   KEY ROTATION (Phase 8)
     PUBLIC_KEYS is a { kid -> JWK } map. To rotate, generate a new
     pair (kid "v2"), add its JWK here, and start issuing "v2" tokens.
     Old "v1" tokens keep verifying until they expire. Remove "v1"
     only after the last v1 token has lapsed.
   ================================================================== */
(function () {
  'use strict';

  /* ---- 1. PUBLIC KEY MAP ----------------------------------------
     kid -> public key JWK. Safe to expose: a public key can only
     VERIFY tokens, never sign them. Generated by Phase 1.            */
  var AIMCQ_PUBLIC_KEYS = {
    "v1": {
  "kty": "RSA",
  "n": "zZHvD60HIdFntfj_fPHxUmD29euMWhnv68Wf5a-wShTO2Iwv-KeNTbojgDqY6MHbZLze3eSafWpNE4TGH2TF1sIKqQzrgEIJ2iZzmoohxyc9KdawrMQxuDCt0458mHR1r7weupxM4NLlJXzOY8BL0PkddE8mX7g9OypLLu6pfONDwzl_Qc_6kslBamAlg3yUedaH7X8lmGynnqbOuXB1UVEX-WyzRqPgiJ5nXlLVdPPAvIquKwrGuF6WpBZksg7hwcK4D_rGFt3o8-i_wVjdXedSETk-IoVGDZWlIGE4_kpSrV58D-RShatkjgsQjHP9VE42jKdXpGnW0xAb4lhbbw",
  "e": "AQAB",
  "alg": "RS256",
  "use": "sig",
  "kid": "v1"
  ...
  /* Where to send blocked users to subscribe. Override via
     AIMCQ_CONFIG.subscribeUrl if your dashboard lives elsewhere.    */
  var AIMCQ_SUBSCRIBE_URL =
    (window.AIMCQ_CONFIG && window.AIMCQ_CONFIG.subscribeUrl) ||
    'https://your-site.example/subscribe';
  ...
  function b64urlToString(str) {
    var bytes = b64urlToUint8(str);
    // decodeURIComponent(escape(...)) is the classic UTF-8-safe atob.
    var bin = '';
  ...
  /* ---- 3. domain matching --------------------------------------
     Roadmap decision: EXACT match for now. localhost and 127.0.0.1
     are allowed so developers can test before going live.          */
  function hostnameAllowed(tokenDomain, currentHost) {
    currentHost = String(currentHost || '').toLowerCase().replace(/^www\./, '');
    tokenDomain = String(tokenDomain || '').toLowerCase().replace(/^www\./, '');
    if (currentHost === 'localhost' || currentHost === '127.0.0.1') return true;
    return currentHost === tokenDomain;
  }

  /* ---- 4. import a JWK as a Web Crypto verify key --------------- */
  function importVerifyKey(jwk) {
  ...
  /* ---- 5. verifyLicenseToken(token) -> Promise<result> ----------
     result: { ok:Boolean, reason:String, payload:Object|null }     */
  function verifyLicenseToken(token) {
    return Promise.resolve().then(function () {
      // Fail closed if Web Crypto is unavailable (very old browser
      // or a non-secure http:// origin where subtle is undefined).
      if (!window.crypto || !window.crypto.subtle) {
        return { ok: false, reason: 'crypto-unavailable', payload: null };
      }
      ...
      // Pick the public key by kid (enables Phase 8 key rotation).
      var jwk = AIMCQ_PUBLIC_KEYS[payload.kid];
      if (!jwk) {
        return { ok: false, reason: 'unknown-kid', payload: payload };
      }

      var data = new TextEncoder().encode(parts[0]); // sign-over-payload-string
      var sig  = b64urlToUint8(parts[1]);

      return importVerifyKey(jwk).then(function (key) {
        return window.crypto.subtle.verify(
          { name: 'RSASSA-PKCS1-v1_5' }, key, sig, data
        );
      }).then(function (sigValid) {
        if (!sigValid) {
          return { ok: false, reason: 'bad-signature', payload: payload };
        }
        // Signature good — now check the claims.
        if (!hostnameAllowed(payload.domain, window.location.hostname)) {
          return { ok: false, reason: 'domain-mismatch', payload: payload };
        }
        var exp = new Date(payload.expiry).getTime();
        if (isNaN(exp)) {
          return { ok: false, reason: 'bad-expiry', payload: payload };
        }
        if (exp < Date.now()) {
        ...
        return { ok: false, reason: 'verify-error', payload: payload };
      });
    });
  }

  /* ---- 6. showLicenseError(containerId, reason) -----------------
     Replaces a quiz container with a styled blocked-state panel.   */
  function showLicenseError(containerId, reason) {
    var el = document.getElementById(containerId);
    if (!el) return;
    var msgMap = {
      'no-token':          'No license token was found for this site.',
      'expired':           'This license has expired.',
      'domain-mismatch':   'This license is not valid for this domain.',
      'bad-signature':     'This license token is invalid.',
      'malformed':         'This license token is invalid.',
      'bad-payload':       'This license token is invalid.',
      'unknown-kid':       'This license token is invalid.',
      'crypto-unavailable':'This browser is too old to verify the license.',
      'verify-error':      'The license could not be verified.'
    };
    var msg = msgMap[reason] || 'A valid license is required to run this quiz.';
    el.innerHTML =
      '<div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;' +
        'max-width:520px;margin:1.5rem auto;padding:2rem 1.75rem;border-radius:14px;' +
        'border:1px solid #e2c9c9;background:#fff7f7;color:#5a2a2a;text-align:center;' +
        'box-shadow:0 1px 3px rgba(0,0,0,.06);">' +
        '<div style="font-size:2rem;line-height:1;margin-bottom:.6rem;">&#128274;</div>' +
        '<div style="font-weight:700;font-size:1.1rem;margin-bottom:.4rem;">' +
          'License required</div>' +
        '<div style="font-size:.95rem;color:#7a4a4a;margin-bottom:1.1rem;">' + msg +
          '</div>' +
        '<a href="' + AIMCQ_SUBSCRIBE_URL + '" target="_blank" rel="noopener" ' +
          'style="display:inline-block;padding:.6rem 1.4rem;border-radius:8px;' +
          'background:#c0392b;color:#fff;text-decoration:none;font-weight:600;' +
          'font-size:.92rem;">Get a license</a>' +
      '</div>';
  }

  /* ---- 7. EXPOSE ON A PRIVATE NAMESPACE ------------------------
     The engine's gated entry points (patched further down in
     aimcq.js) read these. Kept on one object to minimise globals. */
  window.__AIMCQ_LICENSE = {
    verify:        verifyLicenseToken,
    showError:     showLicenseError,
    /* Caches the verification result so a page with several quizzes
       verifies the token only once, not once per container.        */
    _cache:        null,
    check:         function () {
      if (window.__AIMCQ_LICENSE._cache) {
        return Promise.resolve(window.__AIMCQ_LICENSE._cache);
      }
      var token = (window.AIMCQ_CONFIG && window.AIMCQ_CONFIG.licenseToken) || '';
      return verifyLicenseToken(token).then(function (res) {
        window.__AIMCQ_LICENSE._cache = res;
        return res;
      });
    }
  };
...
function _aimcqInitQuizInternal(containerId, rawJSONData, customSettings) {
...
function _aimcqLoadFromDriveInternal(containerId, opts) {
    opts = opts || {};
    var container = document.getElementById(containerId);
    if (!container) return;
    ...
    Promise.all(entries.map(fetchOne)).then(function(results) {
        ...
        _aimcqInitQuizInternal(containerId, merged, opts.settings || {});
...
/* ==================================================================
   LICENSE-GATED PUBLIC ENTRY POINTS  (Phase 5)
   ==================================================================
   These REPLACE the original window.initAimcqQuiz / loadAimcqFromDrive.
   Every external call now verifies the license token FIRST. The check
   is async and its result is cached, so a page with several quizzes
   verifies exactly once. On failure the target container shows the
   styled "license required" panel and the quiz never renders.

   Fails closed: any error path blocks the quiz.
   ================================================================== */
window.initAimcqQuiz = function (containerId, rawJSONData, customSettings) {
    window.__AIMCQ_LICENSE.check().then(function (res) {
        if (res && res.ok) {
            _aimcqInitQuizInternal(containerId, rawJSONData, customSettings);
        } else {
            window.__AIMCQ_LICENSE.showError(containerId, res && res.reason);
        }
    }).catch(function () {
        window.__AIMCQ_LICENSE.showError(containerId, 'verify-error');
    });
};
...
window.loadAimcqFromDrive = function (containerId, opts) {
    window.__AIMCQ_LICENSE.check().then(function (res) {
        if (res && res.ok) {
            _aimcqLoadFromDriveInternal(containerId, opts);
        } else {
            window.__AIMCQ_LICENSE.showError(containerId, res && res.reason);
        }
    }).catch(function () {
